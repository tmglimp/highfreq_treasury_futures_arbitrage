...
def calculate_quantities_with_sma(HEDGES_Combos):
    ...
    SMA = get_acct_dets() * 4
    print(f'SMA => {SMA}')
    ORDERS = calculate_quantities(HEDGES_Combos, SMA)
...
def calculate_quantities(HEDGES_Combos, SMA):
    ...
    HEDGES_Combos['Z_Ln_A'] = np.log(HEDGES_Combos['ln_A_FUT'])
    HEDGES_Combos['ZZ_Ln_A'] = HEDGES_Combos['Z_Ln_A']
    HEDGES_Combos['Z_Ln_B'] = np.log(HEDGES_Combos['ln_B_FUT'])
    HEDGES_Combos['ZZ_Ln_B'] = HEDGES_Combos['Z_Ln_B']
    HEDGES_Combos['Z_Ln_WeightedVol'] = HEDGES_Combos['ZZ_Ln_A'] + HEDGES_Combos['ZZ_Ln_B']
    ...
    if len(unique_rows) >= 3:
        top_row = unique_rows.iloc[0]
        if2risky = unique_rows.iloc[1]
        if2riskyagain = unique_rows.iloc[2]
    else:
        print("Not enough unique rows available.")
        top_row = if2risky = if2riskyagain = None

    ORDERS = [top_row, if2risky, if2riskyagain]
    ...
    config.ORDERS = [
       {
           ...
           "front_ratio": top_row['A_Q_Value'] * top_row['A_Q_sign'],
           "back_conId": top_row['B_FUT_CONID'],
           "back_ratio": top_row['B_Q_Value'] * top_row['B_Q_sign'],
           "quantity": top_row['PairsLCM'],
           "price": -0.8 * (top_row['PairsAdjNetBasis'] - fee_dict['top_total_fees'])
       },
       {
           "front_conId": if2risky['A_FUT_CONID'],
           "front_ratio": if2risky['A_Q_Value'] * if2risky['A_Q_sign'],
           "back_conId": if2risky['B_FUT_CONID'],
           "back_ratio": if2risky['B_Q_Value'] * if2risky['B_Q_sign'],
       ...
       },
       {
           ...
           "front_ratio": if2riskyagain['A_Q_Value'] * if2riskyagain['A_Q_sign'],
